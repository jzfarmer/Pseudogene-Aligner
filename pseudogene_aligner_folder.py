...
def get_gene_symbol_from_record(record):
    symbols = []
    for feat in record.features:
        if feat.type == "gene":
            sym = feat.qualifiers.get("gene", [])
            if sym:
                symbols.append(sym[0])
    if symbols:
        return sorted(symbols, key=len)[0]
    try:
        return record.annotations.get("gene", None) or record.name
    except Exception:
        return record.name
...
def fetch_transcript_and_exons(accession, email):
    ...
    gene_symbol = get_gene_symbol_from_record(record)
    return seq, exons, gene_symbol
...
def normalize_coords_tag(coords):
    # Produce a filesystem-friendly tag like: chr17_55560906-55561577_minus/plus
    chrom, start, end, strand = parse_coords(coords)
    strand_tag = "minus" if strand == 2 else "plus"
    return f"chr{chrom}_{start}-{end}_{strand_tag}"
...
def fetch_pseudo_from_coords(coords, build, email):
    ...
    handle = Entrez.efetch(
        ...
        seq_start=start,
        seq_stop=end,
        strand=strand  # 1=forward, 2=reverse (NCBI returns reverse-complement if 2)
    )
    try:
...
def main():
    ...
    parent_seq, exons, gene_symbol = fetch_transcript_and_exons(transcript_id, args.email)
    if not gene_symbol:
        gene_symbol = (args.gene or transcript_id).split(".")[0]
    ...
    if args.pseudo_fasta:
        ...
        pseudo_tag = f"fasta_{Path(args.pseudo_fasta).stem}"
    else:
        ...
        pseudo_tag = normalize_coords_tag(args.pseudo_coords)
    ...
    # Output folder
    outdir_name = f"{gene_symbol}__{pseudo_tag}"
    outdir = Path(outdir_name)
    outdir.mkdir(parents=True, exist_ok=True)
    print(f"\nOutputs will be saved in: {outdir.resolve()}")

    # Step 3: alignment (local)
    ...
    print(f"Alignment region in parent cDNA (mapped): {begin} to {end} (length {end - begin})")
    ...
    for a, b in zip(aln_parent, aln_pseudo):
        if a != '-' and b != '-':
            ...
            if a.upper() == b.upper():
                matches += 1
    overall_identity_gapless = 100 * matches / valid if valid else 0.0
    # gap-aware (over aligned columns; gaps counted as differences)
    aligned_cols = len(aln_parent)
    identical_bases = sum(1 for a, b in zip(aln_parent, aln_pseudo)
                          if a != '-' and b != '-' and a.upper() == b.upper())
    gap_aware_identity = 100.0 * identical_bases / aligned_cols if aligned_cols else 0.0

    print(f"Overall percent identity (gapless positions only): {overall_identity_gapless:.2f}%")
    print(f"Percent Identity (gap-aware over aligned columns): {gap_aware_identity:.2f}%")
    ...
    for ex in overlapping:
        ...
        # Conservative: matches / exon_len
        identity_per_exon = 100.0 * m / exon_len if exon_len else 0.0
        # Classic (gapless): matches / valid aligned positions
        identity_per_exon_gapless = 100.0 * m / v if v else 0.0
        ...
        per_exon_stats.append({
            ...
            "identity_percent_gapless": identity_per_exon_gapless
        })

        print(f"Exon {ex['exon']}: {ovl_start}–{ovl_end} | len {exon_len} | "
              f"aligned {aligned_len} | matches {m} | gaps(parent) {gp} gaps(pseudo) {gq} | "
              f"valid {v} | cov {coverage:.2f}% | "
              f"ID_exonlen {identity_per_exon:.2f}% | ID_gapless {identity_per_exon_gapless:.2f}%")
    ...
    plt.tight_layout()
    global_png = outdir / f"{args.prefix}_pseudogene_alignment.png"
    plt.savefig(global_png); plt.close()
    print(f"Plot saved to {global_png}")

    # Step 6B: gapped exon track
    ...
    for ex in overlapping:
        ...
        pid_gapless = st.get('identity_percent_gapless', 0.0)
        cov = st.get('coverage_percent', 0.0)
        gap_only = (st.get('valid_positions', 0) == 0)
        label = (f"Exon {ex['exon']} (ID_exonlen {pid:.1f}%, "
                 f"ID_gapless {pid_gapless:.1f}%, {cov:.1f}% cov"
                 + (", gap-only" if gap_only else "") + ")")
    ...
    gapped_png = outdir / f"{args.prefix}_pseudogene_gapped_exons.png"
    ...
    stats_txt = outdir / f"{args.prefix}_pseudogene_alignment_stats.txt"
    with open(stats_txt, "w") as f:
        f.write(f"Alignment Score: {score}\n")
        f.write(f"Alignment Region (cDNA coords): {begin}–{end}\n")
        f.write(f"Alignment Length: {end - begin}\n")
        f.write(f"Percent Identity (gapless positions only): {overall_identity_gapless:.2f}%\n")
        f.write(f"Percent Identity (gap-aware over aligned columns): {gap_aware_identity:.2f}%\n\n")
        ...
        f.write(f"{'Exon':<6}{'Start':>8}{'End':>8}{'Matches':>10}{'Gaps_parent':>12}{'Gaps_Pseudo':>12}"
                f"{'Valid_Pos':>12}{'Exon_Len':>10}{'Aligned_Len':>12}"
                f"{'Coverage(%)':>14}{'ID_exonlen(%)':>14}{'ID_gapless(%)':>14}\n")
        for s in per_exon_stats:
            f.write(f"{s['exon']:<6}{s['start']:>8}{s['end']:>8}"
                    ...
                    f"{s['coverage_percent']:>14.2f}{s['identity_percent']:>14.2f}{s['identity_percent_gapless']:>14.2f}\n")
